module.exports = (db, cfb) => {
        ...
        if (games && games.length) {
            let scoreboard = await cfb.any(`
                    WITH this_week AS (
                        ...
                        FROM game
                        WHERE start_date > (now() - interval '24h')
    ...
    const getTeamsList = async (msg) => {
        let teams = await cfb.any(`
            SELECT t.school
            FROM team AS t
                INNER JOIN conference_team AS ct ON t.id = ct.team_id AND ct.start_year <= 2021 AND ct.end_year IS NULL
            ORDER BY t.school
        `);
    ...
    const getLines = async (msg, team) => {
        ...
        FROM game AS g
            ...
            INNER JOIN game_team AS gt2 ON g.id = gt2.game_id AND gt.id <> gt2.id
            INNER JOIN team AS t2 ON gt2.team_id = t2.id
            INNER JOIN game_lines AS gl ON g.id = gl.game_id AND gl.lines_provider_id = 999999
        WHERE g.start_date > (now() - interval '3d') AND ($1 = LOWER(t.school) OR $1 = LOWER(t2.school))
        ORDER BY g.start_date
        LIMIT 1        
        `, [team]);
        ...
        const embed = {
            color: 0x03bafc,
            title: `Live Odds: ${gameInfo.away_team} ${gameInfo.neutral_site == true ? 'vs' : 'at'} ${gameInfo.home_team}`,
            description: getDisplayDate(gameInfo.start_date),
            fields: [
                {
                    name: 'Spread',
            ...
            footer: {
                text: 'Lines are locked at kickoff'
            }
        };
