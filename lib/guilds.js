module.exports = (db, discord) => {
    ...
    const broadcastCloseGame = async (content) => {
        ...
        broadcastStatus(channels, `:boom:CLOSE GAME ALERT: ${displayScore(content)} (${getClock(content)}) (${getClock(content)}):boom:`)
    }
    ...
    return {
        ...
        onScoreChanged: async (content) => {
            ...
            } else if ((content.status.period == 4 && content.status.clock < 300) || content.status.period > 4) {
