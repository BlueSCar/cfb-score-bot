module.exports = (db, discord) => {
    ...
    const broadcastCloseGame = async (content) => {
        const channels = (await db.any(`
                            SELECT DISTINCT gc.channel_id
                            FROM guild_channel AS gc
                                LEFT JOIN guild_game AS gg ON gg.guild_id = gc.guild_id AND gg.game_id = $1
                            WHERE gg.id IS NULL AND gc.close_game_alerts = $2 AND gc.broadcast_all_fbs <> $3
                        `, [content.id, true, true])).map(r => r.channel_id);
    ...
    const getBroadcastChannels = async (content) => {
        if (content.homeTeam.classification != 'fbs' && content.awayTeam.classification != 'fbs') {
            return [];
        } else {
            return (await db.any(`
                SELECT DISTINCT gc.channel_id
                ...
                WHERE gg.game_id = $1 OR gc.broadcast_all_fbs = $2
